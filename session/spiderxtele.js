...
switch (command) {
    ...
    if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return sam.reply(`Your not authorized to use this command.\nPlease DM ${OWNER_NAME} to grant access.`);
    }
...
${prefix + command} doc (Reply to a document)`;
    ...
    globalConcealing: true
        ...
        await bot.telegram.sendDocument(sam.message.chat.id, {
            source: filePath,
            filename: 'Encrypted By @ELITEPRO.js'
        });
...
case 'connect': {
    ...
    // Read the adminfile and parse it as JSON
    if (!text) {
        return sam.reply('Please provide a number for requesting pairing code. Usage: /connect 234##');
    }
...
case 'runtime':{
    sam.deleteMessage().catch(() => {});
      reply(`ELITEPRO IS ONLINE: ${runtime(process.uptime())}`)
    }
  break
